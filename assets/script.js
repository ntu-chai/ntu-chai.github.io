...
(() => {
  ...
  if (!window.CHAIFrontmatter || typeof window.CHAIFrontmatter.parseFrontmatter !== 'function') {
    const message = 'Could not start the site: frontmatter parser failed to load.';
    console.error(message);
    document.querySelectorAll('[data-content], [data-workshops]').forEach(host => {
      host.innerHTML = '<p class="workshop-load-error">' + message + '</p>';
      host.setAttribute('aria-busy', 'false');
    });
    return;
  }

  const parseFrontmatter = window.CHAIFrontmatter.parseFrontmatter;
